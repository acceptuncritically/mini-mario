...
- 设计约束（新关卡 v2.1）：
  - 恰好 1 个 P、1 个 F；敌人 8 个；金币约 42 个；**钉刺 10~12 组（每组 2~3 格连续，组间距离缩短、更密集）**；中途检查点 2 个（位于约 1/3 与 2/3 处）；
  - 敌人全部站在实心地面；**空中平台列上禁止出现敌人**；深渊 ≥ 4 处且宽度 2~4 格；钉刺铺在平地上（保留可避让空间）；
  - **中段（约 col 75~120）设「上/下双路」**：上路为空中平台链（间隙 ≤5 格、垂直落差 ≤1 格，无敌人无钉刺，平台顶金币更多），下路为地面路线（平坦好走，但有敌人巡逻与钉刺）；
  - 前 20 列为教学区（无敌人无钉刺），之后难度递增；
  - 终点旗帜前 10 列无敌人；检查点所在岛（col 63~69、133~139）无钉刺。
